...
// spelling of them: the same cliResidencyTopology and the same
...
func cliByIDPlan(cityPath, id string, work beads.Store) (storeref.ResolvedPlan, error) {
	return byIDPlanForTopology(cliResidencyTopology(cityPath, nil, work, nil), id)
}
...
func cliByIDBindingOwner(cityPath, id string) (storeref.Owner, bool, error) {
	return byIDBindingOwnerForTopology(cliResidencyTopology(cityPath, nil, newUnprobedWorkResidual(), nil), id)
}
